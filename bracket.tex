...
We consider the pullback of everything to $B'_1\times B'_2$: let $B'$, $B'_I$, $\wt{C}'_A$, $\ov{C}'_A([\pi_1,\pi_2])$, $(\ov{S}_{\sgray}^i)'$, $(\ov{S}_{\sblue}^i)'$, $(\ov{S}_{\sred}^i)'$, $(\ov{S}_{\syellow}^i)'$, $(\ov{S}_{\sorange}^i)'$ be the pullback of $B$, $B'$, $\wt{C}_A$, $\ov{C}_A([\pi_1,\pi_2])$, $\ov{S}_{\sgray}^i$, $\ov{S}_{\sblue}^i$, $\ov{S}_{\sred}^i$, $\ov{S}_{\syellow}^i$, $\ov{S}_{\sorange}^i$, respectively. 

Recall that $B_I$ is constructed by gluing together 3 pieces \cmt{cite Section 2.4 of conftilde}:
$$\mr E_1(3) \times B_2\times[0,\rho),\quad S^{d-1}\times L\times B_1\times B_2,\quad B_1\times \mr E_2(3) \times[0,\rho), $$
where $L\subset\R^2$ is the (closure of the) blue region below:
%where $L$ is the closed region enclosed by 
%$$\{x=0,0\le y\le 1/2\},\quad \{y=0,0\le x\le 1/2\},\quad \{...\}$$ 

\begin{tikzpicture}[scale=3]
\draw[step=0.2,help lines] (0,0) grid (0.6,0.6);
\draw[<->] (0,0.7) -- (0,0) --(0.7,0); 
\node at (0.75,0) {\tiny x}; 
\node at (0,0.75) {\tiny y}; 
\node [left] at (0,1/2) {\tiny $\frac{1}{2}$}; 
\node [left] at (0,1/3) {\tiny $\frac{1}{3}$}; 
\node [below] at (1/2,0) {\tiny $\frac{1}{2}$}; 
\node [below] at (1/3,0) {\tiny $\frac{1}{3}$}; 

\coordinate (o) at (0,0); 
\coordinate (a) at (0.3,1/2); 
\coordinate (e) at (1/2,0.3); 
\coordinate (c) at (0.15,0.15); 
\coordinate (b) at ($(a)!1/3!(o)$);
\coordinate (d) at ($(e)!1/3!(o)$); 
\draw[help lines] (b) -- (0,1/3); 
\draw[help lines] (d) -- (1/3,0); 

\draw[cyan, fill=cyan, fill opacity=0.4] (e) -- (0.5,0) -- (o) -- coordinate[pos=0.3] (q) (0,0.5) -- (a) to (b) to [in=135, out=-119] coordinate[pos=0.8] (p) (c) to [in=209, out=-45] (d) to (e); 

\draw[green!60!black] (a) to (b) to [in=135, out=-119] coordinate[pos=0.8] (p) (c) to [in=209, out=-45] (d) to (e);
\node[green!60!black] at (0.3,0.4) {\tiny $\xi$}; 

\fill[purple] (p) circle [radius=0.01]; 
\node[purple] at ($(p)+(0.05,0.03)$) {\tiny $p$}; 
\fill[purple] (q) circle [radius=0.01]; 
\node[purple] at ($(q)+(-0.05,0)$) {\tiny $q$}; 
\draw[purple] plot [smooth] coordinates {(p) (q)}; 
\node[purple] at (0.07,0.12) {\tiny $l$};
\end{tikzpicture}

We define $\xi$ to be the curvy stratum of $\partial L$, shown as green in the picture. 
...
Let $p$ be a generic point on $\xi$ and $q$ be a generic point on $\{x=0,0<y<1/2\}$. Let $l$ be a generic path going from $P$ to $q$, transverse to $\partial L$ ($l$ is shown in purple in the picture above). 
Denote 
\begin{align*}
M_{p}&:=v\times p\times B'_1\times B'_2\subset S^{d-1}\times \xi\times B_1\times B_2&\subset B'=B'_{t_0},\\
M_{q}&:=v\times q\times B'_1\times B'_2\subset S^{d-1}\times \{x=0,0<y<1/2\}\times B_1\times B_2&\subset B'_0,\\
M_{l}&:=v\times l\times B'_1\times B'_2\subset S^{d-1}\times L\times B_1\times B_2&\subset B'_I. 
\end{align*}
...
By Lemma \ref{stratasubmersion_lmm}, for every stratum $S$ of $\wt{C}'_{V(\Gamma_i)}$, the differential of  $S\to B'_I$ is surjective at every point on $M_l\backslash M_q$, and on $M_q$ the image of this differential covers $T_{S^{d-1}}\times\{x=0,0<y<1/2\}\times B'_1\times B'_2$. So, since the differential of $l$ at $q$ covers the normal direction of $\{x=0,0<y<1/2\}\subset L$, the map $S\to B'_I$ is transverse to $M_l$. 
We denote by $S|_{M_l}$ the fiber product, viewed as a subspace of $S\subset \wt{C}'_{V(\Gamma_i)}$. 
It is a manifold with corners, and 
$$\partial\wt{C}'_{V(\Gamma_i)}=$$
