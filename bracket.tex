...
We consider the pullback of everything to $B'_1\times B'_2$: let $B'$, $B'_I$, $\wt{C}'_A$, $\ov{C}'_A([\pi_1,\pi_2])$, $(\ov{S}_{\sgray}^i)'$, $(\ov{S}_{\sblue}^i)'$, $(\ov{S}_{\sred}^i)'$, $(\ov{S}_{\syellow}^i)'$, $(\ov{S}_{\sorange}^i)'$ be the pullback of $B$, $B_I$, $\wt{C}_A$, $\ov{C}_A([\pi_1,\pi_2])$, $\ov{S}_{\sgray}^i$, $\ov{S}_{\sblue}^i$, $\ov{S}_{\sred}^i$, $\ov{S}_{\syellow}^i$, $\ov{S}_{\sorange}^i$, respectively. 
...
\draw[green!80!black] (a) to (b) to [in=135, out=-119] coordinate[pos=0.8] (p) (c) to [in=209, out=-45] (d) to (e);
\node[green!80!black] at (0.3,0.4) {\tiny $\xi$}; 
\draw[green!80!black] (o) to (0,1/2); 
\node[green!80!black] at (0.05, 1/4) {\tiny $\xi_0$}; 
...
\node[purple] at (0.07,0.12) {\tiny $l_1$};
\draw[purple] (o) to (q); 
\fill[purple] (o) circle [radius=0.01]; 
\node[purple] at ($(-0.04, 0)+0.5*(q)$) {\tiny $l_0$}; 
\end{tikzpicture}
