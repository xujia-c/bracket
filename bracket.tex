...
Before starting the discussion on propagators, we first define a different notion of ``forgetful map''. 

\begin{dfn}\footnote{\textcolor{red}{not great; try to improve the exposition later}}
Let $T$ be a $\{1,2\}$-labeled tree\footnote{The definition obviously extends to the case of an arbitrary number of marked points and forgetting to an arbitrary subset of marked points, but we only need this simple 2 point case here.}, then $S_T\approx\prod_{v\in V(T)}C_{lp(v)\cup cld(v)}(ls(v))$ is a stratum in $\wt{C}_2$. 
Let $v_0\in V(T)$ be the vertex such that $lp(\ge v_0)=\{1,2\}$ and for all $v>v_0$, $lp(\ge v)\subsetneq\{1,2\}$. 
Denote $sp(T):=ls(v_0)$. 
Define 
$$\hat{f}_T:S_T\longrightarrow {C}_2(sp(T))\qquad \hat{f}_T\big((c_v)_{v\in V(T)}\big)=c'_{v_0},$$
where $c'_{v_0}\in C_2(sp(T))$ is obtained from $c_{v_0}$ by forgetting all points except for two: $f_{v_0}(1)$ and $f_{v_0}(2)$. \textcolor{red}{($f_v$ is defined in conftilde, at the beginning of Section 3.3.)}

The above definition extends to $\ov{S}_T$ as well. If $T'$ is a tree such that $S_{T'}\subset\ov{S}_T$, then $T$ can be obtained from $T'$ by contracting some edges (in a unique way). Let $G$ be the subtree of $T'$ consisting of vertices and edges that are contracted to $v_0$.
Let $V'\subset V(T')$ consist of vertices $v$ such that $\tn{lsset}(ls(v))\cap\tn{lsset}(ls(v_0))=\emptyset$ and $|lp(\ge v)\cap\{1,2\}|<2$. 
Define $G'$ to be obtained from $G$ by: for each vertex $v$ in $V'\cap G$, contracting the edge just below $v$. 
Define
$$\hat{f}_T:S_{T'}\longrightarrow \ov{C}_2(sp(T))\qquad \hat{f}_T\big((c_v)_{v\in V(T')}\big)=(c'_{v})_{v\in V(G')},$$
where each $c'_{v}\in C_2(ls(v))$ is obtained from $c_v$ by forgetting all points except for two: $f_v(1)$ and $f_v(2)$. 
We have therefore defined a map 
$$\hat{f}_T:\ov{S}_T\longrightarrow\ov{C}_2(sp(T)).$$
It is easy to verify that $\hat{f}_T$ is smooth using the charts we constructed in Section \textcolor{red}{(conftilde section)}. 
We can make the same definition of $\hat{f}_T$ verbatim with $\wt{C}_2$ replaced by $\ov{C}_2(\pi)$ or $\ov{C}_2(\R^d)$, where $\pi$ is a arbitrary $(M,\infty)$-bundle. 
\end{dfn}
...
\textcolor{red}{(give examples with picture)}
...
\begin{eg}[in $\ov{C}_2(\pi)$, where $\pi$ is an $(M,\infty)$-bundle]
$T_1=$
\begin{tikzpicture}
\node [below] at (0,0) {\tiny{r}};  
...
A {\it propagator} on $\ov{C}_2(\pi)$ (resp. $\ov{C}_2(\R^d)$) is a closed $(d-1)$-form $\omega$ satisfying: there exists a $(d-1)$-form $\omega_0$ on $S^{d-1}\approx\ov{C}_2(\R^d)$ such that $\int_{S^{d-1}}\omega_0=1$ and for every codimension-1 stratum $S\subset\partial\ov{C}_2(\pi)$ (resp. $S\subset\partial\ov{C}_2(\R)$), $\omega|_{S}=\hat{f}_S^*\omega_0$. 
\end{dfn}

This definition is phrased differently from the usual definition of a propagator, see e.g. \cite[Definition 3.9]{Lescop} or \cite[Lemma 2.12]{WatanabeAddendum}, but can easily be seen as equivalent. 
...
All strata in $\ov{S}_{\sred}\cup \ov{S}_{\syellow}\cup \ov{S}_{\sblue}\subset\partial\wt{C}^*_2$ are of the form $S_T$ for a unique $\{1,2\}$-labeled tree $T$ with at least one edge. 
Define $\omega_{S_T}=\hat{f}_T^*\omega_i$, where $i=1$ if $sp(T)=\pi_1$, $i=2$ if $sp(T)=\pi_2$, $i=0$ if $sp(T)=\R^d$, and remove the subscript $i$ if $sp(T)=[\pi_1,\pi_2]$. 
On the remaining part $\ov{S}_\sgray$ of $\partial\wt{C}_2$, which is one single stratum, define $\omega_{S_\sgray}=\omega$, under the natural identification $\ov{S}_\sgray\approx\ov{C}_2([\pi_1,\pi_2])$. 
...
For two trees $T',T$ such that $S_{T'}\subset \ov{S}_T$, the compatibility condition we need to show becomes $\hat{f}_T^*\omega|_{\ov{S}_{T'}}=\hat{f}_{T'}^*\omega$. 
...
\subsection{Propagator on other parts of the boundary of conftilde}\label{propagator2_sec}
...
\subsection{Extend the propagator to the interior}\label{propagatorext_sec}
