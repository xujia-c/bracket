...
%In this paper, we follow \cite[page 3]{Joyce} for the definition of a smooth manifold with corners.
%We call a weakly smooth map in \cite[Definition 3.1]{Joyce} between manifolds with corners a smooth map instead.
%Tangent spaces of manifolds with corner are defined in \cite[Definition 2.2]{Joyce}, and hence tangent bundles, cotangent bundles, and their tensor bundles are defined. 
%We define differential forms on a manifold with corners to be smooth sections of exterior powers of the cotangent bundle. 
...
Let $M$ be a compact manifold with embedded faces. 
...
\item {\it degree-$k$ differential form on $\partial M$}: a compatible collection of forms, $\{\omega_S\}_S$, as in Proposition \ref{formextension_prp}. 
\end{enumerate}
\begin{rmk}
It is easy to see that the $\R$-singular cohomology of $\partial M$ (or more generally, any manifold with corners and ``hinges'') is equivalent to the de Rham cohomology defined using these forms, but we do not need it in the present paper.
\end{rmk}
...
Suppose $M$ is a compact manifold with corners. Then, there exist an open neighborhood $U$ of $\partial M$, a finite open cover $\cU$ of $U$, such that $\cU$ satisfies the condition in Lemma \ref{cechform2_lmm} when we plug in $\partial M$ for $N$, $U$ for $M$, and the inclusion map for $\iota$. 
\end{lmm}

\cite[Definition 35, Theorem 17]{Hajek}
